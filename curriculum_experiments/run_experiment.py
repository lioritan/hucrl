...
def run_on_parameteric_env(steps_per_task, tasks, wrapper, easy_task, state_func):
    ...
    date_string = datetime.datetime.today().strftime('%Y-%m-%d %H') + "tolerant"
...
class TolerantPendulumReward(AbstractModel):
    ...
    def forward(self, state, action, next_state):
        ...
        angle_tolerance = tolerance(cos_angle, lower=0.85, upper=1.0, margin=0.15)
...
if __name__ == "__main__":
    ...
    run_on_parameteric_env(400, 250, PendulumContinuousWrapper(), easy_params, TolerantPendulumReward(action_cost=0.1))
    pass
